          ...
          <section key={run.generatedAt} className="rounded-card border border-rule bg-bg-raised">
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                <span className="block font-display text-[15px] text-text">{run.query}</span>
...
function EngagementChip({ label, comments }: { label: string; comments?: number }) {
  return (
    <span
      ...
      className="inline-flex items-center gap-1 rounded-full bg-bg-overlay px-2 py-0.5 font-mono text-[10px] text-text-faint"
    >
      {label}
...
function EmptyResearch() {
  return (
    <div className="rounded-card border border-dashed border-rule p-5">
      ...
      <pre className="scrollbar-thin overflow-x-auto rounded-md border border-rule bg-bg p-3 font-mono text-[11px] leading-relaxed text-text-dim">
{`# one-time
      ...
      {open && (
        <div className="mt-3 space-y-2">
          <input
            ...
            type="password"
            className="h-9 w-full max-w-xs rounded-md border border-rule bg-bg px-2 text-xs text-text focus:border-accent-border focus:outline-none"
          />
          <textarea
            value={json}
            ...
            className={cn(
              ...
              'font-mono text-[11px] leading-relaxed text-text placeholder:text-text-faint',
