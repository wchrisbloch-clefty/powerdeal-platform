  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        ...
        <CardBody>
          ...
          <div className="grid grid-cols-6 gap-1.5 sm:grid-cols-10">
            {sorted.map((r) => {
              ...
              return (
                ...
                >
                  <span className="font-mono text-[10px] font-medium text-white mix-blend-luminosity">
                    {r.state}
                  </span>
                </div>
              );
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3 border-t border-rule pt-3">
            {RATE_LEGEND.map((l) => (
              <span key={l.label} className="flex items-center gap-1.5 text-[11px] text-text-dim">
              ...
              <thead>
                <tr className="border-b border-rule bg-bg-raised">
                  {['Utility', 'State', 'Industrial rate', 'YoY', 'Accounts'].map((h) => (
                    <th
                      key={h}
                      className="whitespace-nowrap px-3 py-2 text-left font-mono text-[10px] uppercase tracking-wider text-text-faint"
                    >
                      {h}
                    </th>
                  ))}
      ...
      <Card>
        ...
        <div className="scrollbar-thin max-h-[540px] overflow-y-auto">
          <table className="w-full border-collapse text-sm">
            <tbody>
              {sorted.map((r) => {
